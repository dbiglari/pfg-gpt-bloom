...
//bloom_precision temp=1.0;
//bloom_precision minp=0.0;
//int nummatches=40;
...
int gettokencolor(int t,bloom_precision m)
{
  int r,g,b;
...
{
  int j;
  bloom_precision r=0,g=0,b=0;
  for(j=0;j<12*12;j++)
  {
    bloom_precision a=attentions[i*NUMHEADS*NUMLAYERS+j];
    b+=(NUMHEADS*NUMLAYERS-j)*a;
    g+=abs((NUMHEADS*NUMLAYERS)/2-j)*a*2;
    r+=j*a*0;
  }
  bloom_precision max=r>g?r:g;
  max=b>max?b:max;
  max-=255;
  if(max>0)
  {
...
{
  matchToTokens(currwv,matchlist,nummatches,temperature,0);
}
...
{
  int n=0,i,j;
  bloom_precision sum[768];
  for(i=0;i<768;i++) sum[i]=0;
  for(i=0;i<CTXSIZE;i++)
  if(context_tags[i])
  {
    int t=context[i];
    bloom_precision mul=context_tags[i];
    n+=context_tags[i];
    for(j=0;j<768;j++) sum[j]+=mul*UNPKWTE(wte[t*768+j]);
  }
  n=abs(n);
  if(!n) n=1;
  for(j=0;j<768;j++) currwv[j]=sum[j]/(bloom_precision)n;
  update_matchlist();
  matchlist_visible=1;
  cursor_match=-1;
}
...
void randomize_wv()
{
  bloom_precision min[768],max[768];
  int i,j;
  ...
  if(context_tags[i])
  {
    ...
    for(j=0;j<768;j++)
    {
      bloom_precision a=UNPKWTE(wte[t*768+j]);
      if(a<min[j]) min[j]=a;
      if(a>max[j]) max[j]=a;
    }
    howmany++;
...
void autorun_shot()
{
  runModel(currwv,cursor_slot,0,0);
  cursor_slot++;
  if(cursor_slot<CTXSIZE && context[cursor_slot]==emptytoken)
  {
    update_matchlist();
    cursor_match=pickmatch(matchlist,nummatches,minp, false);
...
void ui_init()
{
  ...
  attentions=malloc(CTXSIZE*NUMLAYERS*NUMHEADS*sizeof(bloom_precision));
  outputcache=malloc(CTXSIZE*WVSIZE*sizeof(bloom_precision));
  context_tags=malloc(CTXSIZE*sizeof(char));
  userwte=malloc(WVSIZE*MAXUSERTOKENS*sizeof(wte_t));
  memset(outputcache,0,CTXSIZE*WVSIZE*sizeof(bloom_precision));
  memset(attentions,0,CTXSIZE*NUMLAYERS*NUMHEADS*sizeof(bloom_precision));
...
void handlekey(SDL_keysym*ks)
{
  ...
  if(userinput_visible)
  {
    ...
    if(k==SDLK_RETURN)
    {
      if(userinput_visible==USERINPUT_TOKENIZE)
        tokenize_to_context(userinput,cursor_slot,0,0);
      else
  ...
  if(matchlist_visible)
  {
    ...
    if(u=='r')
    {
      cursor_match=pickmatch(matchlist,nummatches,0.0, false);
      return;
    }
  } else
  {
