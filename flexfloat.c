...
uint_t flexfloat_get_bits(flexfloat_t *a)
{
    ...
    return ((uint_t)flexfloat_sign(a) << (a->desc.exp_bits + a->desc.frac_bits)) + ((uint_t)exp << a->desc.frac_bits) + frac;
}
...
INLINE void ff_cast(flexfloat_t *obj, const flexfloat_t *source, flexfloat_desc_t desc)
{
    ...
    if (StatsEnabled)
        getCastStats(source->desc, desc)->total += 1;
#endif
}
...
INLINE void ff_inverse(flexfloat_t *dest, const flexfloat_t *a)
{
    ...
    if (dest->tracking_fn)
        (dest->tracking_fn)(dest, dest->tracking_arg);
#endif
#ifdef FLEXFLOAT_STATS
    if (StatsEnabled)
        getOpStats(dest->desc)->minus += 1;
#endif
}
...
INLINE void ff_fma(flexfloat_t *dest, const flexfloat_t *a, const flexfloat_t *b, const flexfloat_t *c)
{
    ...
    {
        if (!eff_sub)
        { // in this case, we need to round away from zero
            fexcept_t flags;
...
INLINE void ff_fma_any(flexfloat_t *dest, const flexfloat_t *a, const flexfloat_t *b, const flexfloat_t *c)
{
    ...
    int mode = fegetround();
    bool eff_sub = flexfloat_sign(a) ^ flexfloat_sign(b) ^ flexfloat_sign(c);
    if (((a->desc.frac_bits < NUM_BITS_FRAC) || (b->desc.frac_bits < NUM_BITS_FRAC) || (c->desc.frac_bits < NUM_BITS_FRAC) || (dest->desc.frac_bits < NUM_BITS_FRAC)) && mode == FE_TONEAREST)
    {
        if (!eff_sub)
        { // in this case, we need to round away from zero
            fexcept_t flags;
...
void ff_clear_stats()
{
    ...
    for (i = 0; i < FLEXFLOAT_STATS_MAX_TYPES * FLEXFLOAT_STATS_MAX_TYPES; ++i)
        if (cast_stats[i].key != 0)
            free(cast_stats[i].value);
