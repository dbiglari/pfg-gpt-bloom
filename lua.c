...
int l_runstep(lua_State*L)
{
  int i=lua_tointeger(L,1);
  i--;
  if(i>=0 && i<CTXSIZE) runModel(currwv,i,0,0);
  return 0;
}
...
int l_match(lua_State*L)
{
  ...
  matchToTokens(currwv,matchlist,80,temp,0); // maxnummatches, temp
  return 0;
}
...
{
  bloom_precision minp=0.001;
  int n=lua_gettop(L);
  if(n>=1) nummatches=lua_tointeger(L,1);
  if(n>=2) minp=lua_tonumber(L,2);
  if(nummatches>MAXNUMMATCHES) nummatches=MAXNUMMATCHES;
  int match=pickmatch(matchlist,nummatches,minp,false,0);
  lua_pop(L,n);
