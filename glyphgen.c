...
#ifdef HAVE_SDL
int makecol(bloom_precision r,bloom_precision g,bloom_precision b)
{
  if(r<0) r=0;
  if(r>1) r=1;
  if(g<0) g=0;
...
{  
  ...
  int col0,col1,colbg;
  {
  bloom_precision pos_locsum=0,pos_sum=0.0001,neg_locsum=0,neg_sum=0.0001;
  bloom_precision pos_ctr=0,neg_ctr=0,max=0;
  for(x=0;x<WVSIZE;x++)
  {
    bloom_precision a=fabs(wv0[x%WVSIZE]);
    if(a>max)max=a;
  ...
  bloom_precision pos_avgloc = pos_locsum/pos_sum;
  bloom_precision neg_avgloc = neg_locsum/neg_sum;
  bloom_precision posvsneg   = pos_sum/neg_sum;
//  fprintf(stderr,"pos_avgloc %f neg_avgloc %f posvsnegctr %f posvsneg %f\n",
//    pos_avgloc,neg_avgloc,pos_ctr-neg_ctr,posvsneg);
  bloom_precision r0,g0,b0,r1,g1,b1,r2,g2,b2;
  ...
  bloom_precision spotlim=(max*3+(pos_sum/pos_ctr))/4;
  if(spotlim<quanter_wte) spotlim=quanter_wte;
  for(x=0;x<1024;x++)
  {
    bloom_precision a=wv0[x%WVSIZE];
  ...
  for(x=0;x<32;x++)
  {
    bloom_precision r2=(y-16)*(y-16)+(x-16)*(x-16);
    bloom_precision a = wv0[(y*32+x)%WVSIZE]/sqrt(r2+4.0);
    if(a>1.0) a=1.0;
    if(a<-1.0) a=-1.0;
...
{
  bloom_precision wv[WVSIZE];
  int i;
  for(i=0;i<WVSIZE;i++) wv[i]=UNPKWTE(wv0[i]);
  renderwordvec(wv,x0,y0,dim);
}

void renderlayernode(bloom_precision*wv0,bloom_precision*att,int numheads,int x0,int y0)
{
  int i,j,x,y;
